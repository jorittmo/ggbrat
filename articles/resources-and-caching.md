...
every resource. The `aliases` column contains familiar alternative
names. Derived forms of the same atlas—such as its annotation, mesh, and
premade ggbrat RDS—share the citation of the atlas from which they were
made.
...
Aliases are matched exactly before fuzzy partial matching. Very short
aliases, such as `"dk"`, only participate in exact matching so that
short searches do not produce a pile of accidental matches.

When several resources match in an interactive R session, ggbrat
...
In scripts, package checks, and CI, ambiguous matching stops and lists
the candidates instead of waiting for input or silently choosing one.
