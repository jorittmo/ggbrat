...
`reposition_atlas()` infers the plotted panels in the order they first appear
in the data and assigns them `A`, `B`, `C`, and so on. For surface atlases,
each view/hemisphere combination is a panel. Volumetric atlases simply use
...
glass-cortex layers. Use `.`, `#`, or a literal space for an empty grid cell:

```{r}
schaefer <- reposition_atlas(schaefer, layout = "AB.\n.CD")
```

The `horizontal_gap` and `vertical_gap` arguments control the space between
panel bounding boxes. Inspect `attr(schaefer, "layout_key")` to see which
view and hemisphere was assigned to each letter.

## Resize a complete atlas

When combining atlases that occupy different coordinate ranges you can scale one of
them as a complete object to get make sure they are comparable in size. 

`atlas_size()` scales the entire layout around its centre, including the
distances between views. It applies the same transformation to all spatial
layers in the atlas object, so polygons, shading, silhouettes, and glass-cortex
points remain aligned. Values above one enlarge the atlas; values between zero
and one shrink it.
...
When building an atlas from an SVG drawing the resulting object will look a bit different from
surface based atlases. The result will be a single feature collection with the regions you have defined in the drawing.

There are two svg atlases of the MTL lobe included in the package (drawn by Anika Wuestefeld) and these can be
plotted like so:
