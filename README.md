...
This package is about versatility, but within the limits of ggplot. As brain imagers favoring R (and ggplot) this is
...
So, the package separates atlas creation from atlas use. Creating a surface atlas
uses Python to determine which mesh vertices are visible from a chosen camera
angle. Once an atlas has been built and saved as an RDS file, plotting and
...
ggbrat keeps large data files outside the R package and downloads only what is
requested. The main helpers are:

```r
download_atlas("Melbourne_S1")
download_annotation("aparc")
download_surface("fsaverage_inflated", type = "cortical")
download_surface("Melbourne_S1", type = "subcortical")
download_volume_atlas("Melbourne_S1")
```
